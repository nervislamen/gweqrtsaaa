...
module.exports = {
  ...
  if (!victim) {
    ...
    if(kisi) {
      ...
      message.channel.send(`Hata: Lütfen bir üye veya sebep giriniz!  __Örn:__  \`${client.sistem.a_Prefix}ban @acar/ID <sebep>\``).then(x => x.delete({timeout: 5000}));
    };
    return message.reply(`Hata: Lütfen bir üye veya sebep giriniz!  __Örn:__  \`${client.sistem.a_Prefix}ban @acar/ID <sebep>\``).then(x => x.delete({timeout: 5000}));
  };
